# Terraform AWS [S3] ![](https://img.shields.io/github/actions/workflow/status/wearetechnative/terraform-aws-s3/lint.yaml?branch=main&style=plastic&label=lint) ![](https://img.shields.io/github/actions/workflow/status/wearetechnative/terraform-aws-s3/security-scan.yaml?branch=main&style=plastic&label=security)
...
The module supports CORS (Cross-Origin Resource Sharing) configuration for web applications that need to access S3 resources from browsers. Use the `cors_rules` variable to define CORS rules.

See [examples/CORS.md](./examples/CORS.md) for detailed examples including:
- Simple read-only access for web applications
...
S3 replication allows you to automatically copy objects between buckets for disaster recovery, compliance, or latency optimization.
...
Each example includes complete, runnable Terraform configurations with:
- Full infrastructure setup (buckets, KMS keys, IAM roles)
...
| <a name="input_cors_rules"></a> [cors\_rules](#input\_cors\_rules) | CORS configuration rules for the S3 bucket. Allows web browsers to make cross-origin requests to the bucket. Each rule specifies allowed origins, methods, and headers. | <pre>list(object({<br/>    allowed_headers = list(string)<br/>    allowed_methods = list(string)<br/>    allowed_origins = list(string)<br/>    expose_headers  = optional(list(string))<br/>    max_age_seconds = optional(number)<br/>  }))</pre> | `[]` | no |
...
| <a name="input_lifecycle_configuration"></a> [lifecycle\_configuration](#input\_lifecycle\_configuration) | Object Lifecycle rules configuration. | <pre>map(object({<br>    status = string<br>    bucket_prefix = string<br>    transition = object({<br>      storage_class = string<br>      transition_days = number<br>    })<br>    expiration_days = object({<br>      days = number<br>      expired_object_delete_marker = bool<br>    })<br>    noncurrent_version_expiration = object({<br>        newer_noncurrent_versions = number<br>        noncurrent_days = number<br>    })<br>    noncurrent_version_transition = object({<br>        newer_noncurrent_versions = number<br>        noncurrent_days = number<br>        storage_class = string<br>    })<br>    abort_incomplete_multipart_upload = object({<br>        days_after_initiation = number<br>    })<br>  }))</pre> | `{}` | no |
